...
const RightsideDataPanel: React.FC<RightsideDataPanelProps> = ({ darkMode }) => {
  const {currentData, baseMonthlyUsage, baseYearlyUsage, maxRealTimeTotalKW} = useEMS();
    ...
    <div className="absolute right-4 top-25 bottom-10 min-w-[270px] w-[20%] h-[85%] z-10 flex flex-col gap-4 pointer-events-none">
      ...
      <div className="relative hud-panel h-[12%] flex gap-2 items-center justify-center">
        <div className="flex flex-col items-center px-4 border-r border-white/10">
          ...
          <span className="text-md 2xl:text-xl font-bold text-[#84ebf8]">{baseMonthlyUsage.toFixed(0)} <small className="text-8 font-normal">kWh</small></span>
        </div>
        <div className="flex flex-col items-center px-4">
          ...
          <span className="text-md 2xl:text-xl font-bold text-[#84ebf8]">{baseYearlyUsage.toFixed(0)} <small className="text-8 font-normal">kWh</small></span>
        </div>
      </div>
      {/*中段：需量監控 */}
      <Card className={`rounded-none hud-panel h-[16%] p-4 relative pointer-events-auto ${darkMode ? 'bg-transparent' : 'bg-white/40'}`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col">
            ...
            <span className="text-md 2xl:text-xl font-bold text-[#84ebf8]">{realTimeTotalKW.toFixed(2)} <small className="text-8 font-normal">kW</small></span>
          </div>
          <div className="flex flex-col">
            <span className="text-md 2xl:text-xl">本月最大需量</span>
            <span className="text-md 2xl:text-xl font-bold text-[#84ebf8]">{maxRealTimeTotalKW.toFixed(2)} <small className="text-8 font-normal">kW</small></span>
          </div>
        </div>
      </Card>
      ...
      <Card className={`rounded-none hud-panel h-[20%] p-4 pointer-events-auto relative overflow-hidden ${darkMode ? 'bg-transparent' : 'bg-white/60'}`}>
        ...
        <div className="flex flex-col gap-2 overflow-y-auto">
          <div className="flex justify-between items-end border-b border-white/5 pb-2">
            ...
            <span className="text-md font-bold text-[#84ebf8]">{ (baseMonthlyUsage/12392).toFixed(3) }<small className="text-8 font-normal text-cyan-400">kWh/m²</small></span>
          </div>
          <div className="flex justify-between items-end border-b border-white/5 pb-2">
            <span className="text-sm 2xl:text-md ">本月人均能耗</span>
            <span className="text-md font-bold text-[#84ebf8]"> {(baseMonthlyUsage/1200).toFixed(3)}<small className="text-8 font-normal text-cyan-400">kWh/人</small></span>
          </div>
        </div>
      </Card>
      {/* 下段：趨勢圖切換 */}
