...
const ResponseHistoryModal = ({ onClose, history, darkMode }: any) => {
  ...
  return (
    ...
    >
      ...
      >
        <h3 className="text-lg font-bold mb-4">{isClient ? t("response_history") : "Response History"}</h3>

        {history.length === 0 ? (
          <p className="text-gray-400">{isClient ? t("no_history") : "No history available for this response."}</p>
        ) : (
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-medium">
                    {isClient ? t("action_by") : "Action by"} <span className="font-bold">{record.user}</span>
                  </span>
                ...
                {record.action === 'deleted' ? (
                  <div className="text-sm">
                    <p className="font-semibold text-red-500">{isClient ? t("response_deleted") : "Response Deleted"}</p>
                    {record.oldText && <p><strong>{isClient ? t("deleted_text") : "Deleted Text"}:</strong> {record.oldText}</p>}
                    {record.oldImageUrl && <p><strong>{isClient ? t("deleted_image") : "Deleted Image"}:</strong> <a href={record.oldImageUrl} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">View Image</a></p>}
                  </div>
