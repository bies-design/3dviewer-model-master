...
interface MobileSearchPanelProps {
    ...
    isGhost: boolean;
    onGhostChange: (enable?: boolean) => void;
    darkMode: boolean;
...
export type MobileSearchPanelRef = {
addItemToGroup: (groupId: number, item: TResultItem) => void;
};

const MobileSearchPanel = forwardRef<MobileSearchPanelRef, MobileSearchPanelProps>(({ components, isGhost, onGhostChange, darkMode, loadedModelIds }, ref) => {
...
const executeSearch = useCallback(async (queries: TQueryRow[]) => {
    ...
    if (foundElements.length > 0) {
        ...
        await highlighter.highlightByID("select", finalResult, true, true);
        onGhostChange(true);
        await hider.set(true);
